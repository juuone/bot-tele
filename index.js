  ...
  return {
    inline_keyboard: [
      ...
      [
        ...
        { text: "📺 Atur YouTube", callback_data: "admin:setyoutube" },
      ],
      [
        { text: "ℹ️ Atur Info Bot", callback_data: "admin:setinfo" },
        { text: "📁 Kelola File", callback_data: "admin:files" },
      ],
      [
...
function delKB() {
  return {
    inline_keyboard: [
      [
        { text: "🗑️ Hapus Pengumuman", callback_data: "admindel:ann" },
        { text: "🗑️ Hapus YouTube", callback_data: "admindel:youtube" },
      ],
  ...
  for (const c of Object.values(chats)) {
    try {
      ...
      if (mode === "text")
        ...
        r = await TG(token, "sendPhoto", { chat_id: c.id, photo: extra.photo, caption: extra.caption, parse_mode: "HTML" });
      else if (mode === "video")
        r = await TG(token, "sendVideo", { chat_id: c.id, video: extra.video, caption: extra.caption, parse_mode: "HTML" });
      if (r && r.ok) ok++;
      else {
        fail++;
...
async function handlePending(token, msg, env, adminId) {
  ...
  const infoActions = { setann: ["ann", "Pengumuman"], setyoutube: ["youtube_info", "Info YouTube"], setinfo: ["bot_info", "Info Bot"] };
...
async function handleCB(token, q, env, adminId) {
  ...
  if (data.startsWith("f:")) {
    ...
    try {
      ...
      return TG(token, "sendMessage", { chat_id: chatId, text: "❌ Gagal mengirim file." });
    }
  }

  if (data.startsWith("cmd:")) {
    const cfg = await getCfg(env);
    const resp = cfg.custom_commands?.[data.slice(4)];
  ...
  if (data.startsWith("admindel:")) {
    ...
    if (key) {
      ...
      delete cfg[key];
      await saveCfg(env, cfg);
      return edit("✅ <b>Data dihapus!</b>", { inline_keyboard: [[{ text: "🔙 Panel Admin", callback_data: "admin:main" }]] });
    }
  }
}
...
async function handleWebhook(req, env) {
  ...
  if (u.message) {
    ...
    if (await handlePending(token, msg, env, adminId)) return new Response("OK");
    ...
    if (/^\/start/i.test(text) || /^\/menu/i.test(text)) {
      ...
      TG(token, "sendMessage", { chat_id: chat.id, parse_mode: "HTML", text: `✅ Kategori <b>${n}</b> ditambahkan!` });
    } else if (/^\/delcat /i.test(text) && isAdmin) {
      const n = text.replace(/^\/delcat /i, "").trim();
      const m = await getMenu(env);
      delete m[n];
      await saveMenu(env, m);
      TG(token, "sendMessage", { chat_id: chat.id, parse_mode: "HTML", text: `✅ Kategori <b>${n}</b> dihapus.` });
      ...
      TG(token, "sendMessage", { chat_id: chat.id, parse_mode: "HTML", text: `<b>📖 Bantuan</b>\n\n/start • /menu • /help${isAdmin ? "\n\n<b>🔑 Admin:</b>\n/admin • /broadcast • /addcat • /delcat • /addfile • /listcat" : ""}` });
    }
  }
...
async function handleAPI(req, env, url) {
  ...
  try {
    ...
    if (path === "update-chat") {
      ...
      const c = await getChats(env);
      if (c[String(id)]) {
        if (notes !== undefined) c[String(id)].notes = notes;
        if (display_name !== undefined) c[String(id)].display_name = display_name;
        await saveChats(env, c);
      }
      return jn({ ok: true });
    }
    if (path === "remove-chat") {
      await removeChat(env, (await req.json()).id);
      return jn({ ok: true });
    }
    if (path === "files") return jn({ ok: true, menu: await getMenu(env) });
    if (path === "add-cat") {
      const m = await getMenu(env);
      m[(await req.json()).name] = {};
      await saveMenu(env, m);
      return jn({ ok: true });
    }
    if (path === "del-cat") {
      const m = await getMenu(env);
      delete m[(await req.json()).name];
      await saveMenu(env, m);
      return jn({ ok: true });
    }
    if (path === "add-file") {
    ...
    if (path === "edit-file") {
      ...
      if (m[b.cat]?.[b.ver]) {
        ...
        if (b.new_ver && b.new_ver !== b.ver) {
          m[b.cat][b.new_ver] = m[b.cat][b.ver];
          delete m[b.cat][b.ver];
        }
        await saveMenu(env, m);
      }
      return jn({ ok: true });
    ...
    if (path === "del-file") {
      const b = await req.json();
      const m = await getMenu(env);
      if (m[b.cat]) delete m[b.cat][b.ver];
      await saveMenu(env, m);
      return jn({ ok: true });
    }
    ...
    if (path === "save-cfg") {
      const b = await req.json();
      const cfg = await getCfg(env);
      for (const k of ["bot_info", "youtube_info", "ann"]) if (b[k] !== undefined) cfg[k] = b[k];
      await saveCfg(env, cfg);
      return jn({ ok: true });
    }
    if (path === "del-cfg") {
      const cfg = await getCfg(env);
      delete cfg[(await req.json()).key];
      await saveCfg(env, cfg);
      return jn({ ok: true });
    }
    ...
    if (path === "del-button") {
      ...
      const cfg = await getCfg(env);
      if (cfg.custom_buttons?.[idx]) {
        if (cfg.custom_buttons[idx].trigger && cfg.custom_commands) delete cfg.custom_commands[cfg.custom_buttons[idx].trigger];
    ...
    if (path === "del-command") {
      const cfg = await getCfg(env);
      delete cfg.custom_commands[(await req.json()).trigger];
      await saveCfg(env, cfg);
      return jn({ ok: true });
    }
    if (path === "broadcast") {
      const b = await req.json();
      let list;
      if (b.chat_ids?.length) {
        const ac = await getChats(env);
        list = b.chat_ids.map(id => ac[String(id)]).filter(Boolean);
      } else {
      ...
      for (const chat of list) {
        try {
          let r;
          if (b.mode === "text") r = await TG(T, "sendMessage", { chat_id: chat.id, text: b.text, parse_mode: "HTML" });
          else if (b.mode === "photo") r = await TG(T, "sendPhoto", { chat_id: chat.id, photo: b.photo, caption: b.caption, parse_mode: "HTML" });
          else if (b.mode === "video") r = await TG(T, "sendVideo", { chat_id: chat.id, video: b.video, caption: b.caption, parse_mode: "HTML" });
          if (r?.ok) ok++;
          else { fail++; if (r?.error_code === 403) await removeChat(env, chat.id); }
        } catch { fail++; }
      }
      return jn({ ok: true, success: ok, failed: fail });
    }
    if (path === "setup-webhook") {
      const r = await TG(T, "setWebhook", {
        url: url.origin + "/webhook",
        secret_token: env.WEBHOOK_SECRET,
        allowed_updates: ["message", "callback_query", "my_chat_member"],
      });
      return jn({ ok: r.ok, result: r });
    }
    return jn({ ok: false, error: "Not found" }, 404);
  } catch (e) {
    return jn({ ok: false, error: e.message }, 500);
  }
}
...
export default {
  async fetch(req, env) {
    ...
    // Dashboard (disajikan dari KV key "dashboard_html")
    if (url.pathname === "/dashboard") {
      const html = await env.BOT_KV.get("dashboard_html");
      if (html) return new Response(html, { headers: { "Content-Type": "text/html;charset=UTF-8" } });
      return new Response("Dashboard not uploaded yet. Put HTML in KV key 'dashboard_html'.", { status: 404 });
    }
