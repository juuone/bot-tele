...
async function handlePending(token, msg, env, adminId) {
  ...
  if (p.action === "broadcast") {
    ...
    return TG(token, "sendMessage", { chat_id: chatId, parse_mode: "HTML",
  ...
  if (p.action === "setann" || p.action === "setyoutube" || p.action === "setinfo") {
    ...
    await setPending(env, uid, null);
    return TG(token, "sendMessage", { chat_id: chatId, parse_mode: "HTML",
      text: "✅ <b>" + labelMap[p.action] + " berhasil diperbarui!</b>", reply_markup: adminKB() });
  }
...
async function handleCB(token, q, env, adminId) {
  ...
  if (data === "admin:users") {
    ...
    const lines = chats.map((u,i)=>(i+1)+". <b>"+(u.title||"?")+
      "</b>"+(u.username?" @"+u.username:"")+(u.notes?" — <i>"+u.notes+"</i>":"")+"\n   <code>"+u.id+"</code>").join("\n");
    return edit("👥 <b>Daftar Pengguna</b> ("+chats.length+" ditampilkan)\n━━━━━━━━━━━━━━━━━━━━\n"+(lines||"Kosong.")+"\n━━━━━━━━━━━━━━━━━━━━\nℹ️ Kelola lengkap di Web Dashboard.",
      { inline_keyboard: [[{ text: "🔙 Panel Admin", callback_data: "admin:main" }]] });
  }
  if (data === "admin:files") {
...
function json(data, s) { return new Response(JSON.stringify(data), { status: s||200, headers: {"Content-Type":"application/json","Access-Control-Allow-Origin":"*"}}); }

// ── API ───────────────────────────────────────────────────────
async function handleAPI(req, env, url) {
  const path = url.pathname.replace(/^\/api\//,"");
  if (path==="check") return json({ pw: !!env.DASHBOARD_PASSWORD, token: !!env.BOT_TOKEN, kv: !!env.BOT_KV });
  if (!checkAuth(req, env)) return json({ ok:false, error:"Unauthorized" }, 401);
  const token = env.BOT_TOKEN;

  if (path==="chats") return json({ ok:true, chats: await getChats(env) });
  if (path==="update-chat") {
    const { id, notes, display_name } = await req.json();
    const chats = await getChats(env);
    if (chats[String(id)]) {
      if (notes !== undefined) chats[String(id)].notes = notes;
      if (display_name !== undefined) chats[String(id)].display_name = display_name;
      await saveChats(env, chats);
    }
    return json({ ok:true });
  }
  if (path==="remove-chat") { const { id } = await req.json(); await removeChat(env, id); return json({ ok:true }); }
  if (path==="files") return json({ ok:true, menu: await getMenu(env) });
  if (path==="add-cat") { const { name } = await req.json(); const m = await getMenu(env); if(!m[name]){m[name]={};await saveMenu(env,m);} return json({ok:true}); }
  if (path==="del-cat") { const { name } = await req.json(); const m = await getMenu(env); delete m[name]; await saveMenu(env,m); return json({ok:true}); }
  if (path==="add-file") {
    const b = await req.json(); const m = await getMenu(env);
    if (!m[b.cat]) m[b.cat]={};
    m[b.cat][b.ver] = { file_id:b.file_id, file_type:b.file_type||"document", caption:b.caption||"", added_at:new Date().toISOString() };
    await saveMenu(env,m); return json({ok:true});
  }
  if (path==="edit-file") {
    const b = await req.json(); const m = await getMenu(env);
    if (m[b.cat] && m[b.cat][b.ver]) {
      if (b.caption !== undefined) m[b.cat][b.ver].caption = b.caption;
      if (b.new_ver && b.new_ver !== b.ver) { m[b.cat][b.new_ver] = m[b.cat][b.ver]; delete m[b.cat][b.ver]; }
      await saveMenu(env,m);
    }
    return json({ok:true});
  }
  if (path==="del-file") { const { cat,ver } = await req.json(); const m = await getMenu(env); if(m[cat]) delete m[cat][ver]; await saveMenu(env,m); return json({ok:true}); }
  if (path==="cfg") return json({ ok:true, cfg: await getCfg(env) });
  if (path==="save-cfg") {
    const b = await req.json(); const cfg = await getCfg(env);
    const allowed = ["bot_info","youtube_info","ann"];
    for (const k of allowed) { if (b[k]!==undefined) cfg[k] = b[k]; }
    await saveCfg(env, cfg); return json({ok:true});
  }
  if (path==="del-cfg") {
    const { key } = await req.json(); const cfg = await getCfg(env);
    delete cfg[key]; await saveCfg(env, cfg); return json({ok:true});
  }
  if (path==="menu-buttons") { const cfg = await getCfg(env); return json({ok:true, buttons: cfg.custom_buttons||[]}); }
  if (path==="add-button") {
    const b = await req.json(); const cfg = await getCfg(env);
    if (!cfg.custom_buttons) cfg.custom_buttons = [];
    cfg.custom_buttons.push({ text:b.text, type:b.type, url:b.url||"", data:b.data||"", trigger:b.trigger||"", response:b.response||"" });
    if (b.type==="command" && b.trigger && b.response) {
      if (!cfg.custom_commands) cfg.custom_commands = {};
      cfg.custom_commands[b.trigger] = b.response;
    }
    await saveCfg(env, cfg); return json({ok:true});
  }
  if (path==="del-button") {
    const { idx } = await req.json(); const cfg = await getCfg(env);
    if (cfg.custom_buttons && cfg.custom_buttons[idx]) {
      const btn = cfg.custom_buttons[idx];
      if (btn.trigger && cfg.custom_commands) delete cfg.custom_commands[btn.trigger];
      cfg.custom_buttons.splice(idx, 1);
      await saveCfg(env, cfg);
    }
    return json({ok:true});
  }
  if (path==="commands") { const cfg = await getCfg(env); return json({ok:true, commands: cfg.custom_commands||{}}); }
  if (path==="add-command") {
    const { trigger, response } = await req.json(); const cfg = await getCfg(env);
    if (!cfg.custom_commands) cfg.custom_commands = {};
    cfg.custom_commands[trigger] = response; await saveCfg(env, cfg); return json({ok:true});
  }
  if (path==="del-command") {
    const { trigger } = await req.json(); const cfg = await getCfg(env);
    if (cfg.custom_commands) delete cfg.custom_commands[trigger]; await saveCfg(env, cfg); return json({ok:true});
  }
  if (path==="broadcast") {
    const b = await req.json();
    const chats = await getChats(env); let list = Object.values(chats);
  ...
  if (path==="setup-webhook") {
    const r = await TG(token,"setWebhook",{ url:url.origin+"/webhook", secret_token:env.WEBHOOK_SECRET, allowed_updates:["message","callback_query","my_chat_member"] });
...
function dashboardHTML(origin, k) {
  ...
  return '<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MCPatch Dashboard</title>'
  +'<link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">'
  +'<style>*{margin:0;padding:0;box-sizing:border-box}:root{--bg:#060d1f;--s:#0d1426;--c:#111827;--b:rgba(59,130,246,.15);--b2:rgba(255,255,255,.06);--a:#3b82f6;--p:#8b5cf6;--g:#22c55e;--r:#ef4444;--y:#f59e0b;--t:#e2e8f0;--m:#64748b}'
