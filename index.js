...
<div class="wrapper">
  <nav class="sidebar">
    <div class="nav-item active" onclick="go('overview')"><span class="nav-icon">📊</span> Overview</div>
    <div class="nav-item" onclick="go('broadcast')"><span class="nav-icon">📢</span> Broadcast</div>
    <div class="nav-item" onclick="go('chats')"><span class="nav-icon">💬</span> Chat List</div>
    <div class="nav-item" onclick="go('files')"><span class="nav-icon">📁</span> File Manager</div>
    <div class="nav-item" onclick="go('settings')"><span class="nav-icon">⚙️</span> Settings</div>
  </nav>
...
function loadStats(){
  fetch(BASE+'/chat-list',{headers:h()}).then(r=>r.json()).then(d=>{
    if(!d.ok) return;
    const cs=Object.values(d.chats);
    allChats=cs;
...
function renderChats(list){
  ...
  el.innerHTML='<table><thead><tr><th>ID</th><th>Nama</th><th>Tipe</th><th>Username</th><th>Bergabung</th><th></th></tr></thead><tbody>'+
    list.map(c=>'<tr>'+
      ...
      '<td><button class="btn btn-danger btn-sm" onclick="delChat('+c.id+')">🗑️</button></td>'+
    '</tr>').join('')+'</tbody></table>';
}
function filterChats(){
  const q=document.getElementById('chatQ').value.toLowerCase();
  renderChats(allChats.filter(c=>(c.title||'').toLowerCase().includes(q)||(c.username||'').toLowerCase().includes(q)||String(c.id).includes(q)));
}
function delChat(id){
  if(!confirm('Hapus chat '+id+' dari list?')) return;
  fetch(BASE+'/remove-chat',{method:'POST',headers:h(),body:JSON.stringify({chat_id:id})})
    .then(r=>r.json()).then(d=>{
...
function loadFiles(){
  fetch(BASE+'/get-files',{headers:h()}).then(r=>r.json()).then(d=>{
    ...
    sel.innerHTML=Object.keys(d.menu).map(c=>'<option value="'+esc(c)+'">'+esc(c)+'</option>').join('');
  });
}
function renderTree(menu){
  ...
  el.innerHTML=cats.map(cat=>{
    const vers=Object.keys(menu[cat]);
    return '<div class="tree-cat">'+
      '<div class="tree-cat-header"><span>📁 '+esc(cat)+'</span>'+
      '<button class="btn btn-danger btn-sm" onclick="delCat(\''+esc2(cat)+'\')">🗑️ Hapus Kategori</button></div>'+
      (vers.length?vers.map(v=>'<div class="tree-file"><span>📦 '+esc(v)+' <small style="color:var(--muted)">['+menu[cat][v].file_type+']</small></span>'+
        '<button class="btn btn-danger btn-sm" onclick="delFile(\''+esc2(cat)+'\',\''+esc2(v)+'\')">🗑️</button></div>').join(''):
...
function addCat(){
  ...
  fetch(BASE+'/add-category',{method:'POST',headers:h(),body:JSON.stringify({name})})
    .then(r=>r.json()).then(d=>{
...
function delCat(name){
  if(!confirm('Hapus kategori "'+name+'" dan semua file di dalamnya?')) return;
  fetch(BASE+'/delete-category',{method:'POST',headers:h(),body:JSON.stringify({name})})
    .then(r=>r.json()).then(d=>{if(d.ok){alert2('Kategori dihapus!');loadFiles();}else alert2('Error','err');});
}
function addFile(){
  ...
  fetch(BASE+'/add-file',{method:'POST',headers:h(),body:JSON.stringify({category:cat,version:ver,file_id:fid,file_type:ft,caption:cap})})
    .then(r=>r.json()).then(d=>{if(d.ok){alert2('File disimpan!');loadFiles();}else alert2('Error: '+d.error,'err');});
}
function delFile(cat,ver){
  if(!confirm('Hapus file '+ver+'?')) return;
  fetch(BASE+'/delete-file',{method:'POST',headers:h(),body:JSON.stringify({category:cat,version:ver})})
    .then(r=>r.json()).then(d=>{if(d.ok){alert2('File dihapus!');loadFiles();}else alert2('Error','err');});
}
...
async function sendBroadcast(){
  ...
  let payload={mode:curMode,target};
  if(curMode==='text'){
  ...
  const d=await fetch(BASE+'/broadcast',{method:'POST',headers:h(),body:JSON.stringify(payload)}).then(r=>r.json());
...
function loadSettings(){
  fetch(BASE+'/get-settings',{headers:h()}).then(r=>r.json()).then(d=>{
    if(!d.ok) return;
...
function saveSetting(key,elId,label){
  ...
  fetch(BASE+'/save-setting',{method:'POST',headers:h(),body:JSON.stringify({key,value})})
    .then(r=>r.json()).then(d=>{if(d.ok)alert2(label+' berhasil disimpan!');else alert2('Gagal!','err');});
}
function setupWebhook(){
  fetch(BASE+'/setup-webhook',{method:'POST',headers:h()})
    .then(r=>r.json()).then(d=>{
